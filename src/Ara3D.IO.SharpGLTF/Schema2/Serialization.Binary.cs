...
namespace SharpGLTF.Schema2
{
    static class _BinarySerialization
    {
        ...
        public static IReadOnlyDictionary<UInt32, Byte[]> ReadBinaryFile(Stream stream)
        {
            ...
            using (var binaryReader = new BinaryReader(stream, Encoding.ASCII))
            {
                ...
                void _checkCanRead(long count)
                {
                    ...
                    if (count > int.MaxValue) throw new Exception($"{count} bytes to read exceeds maximum capacity.");
                    if (remaining < count) throw new Exception("unexpected End of GLB block.");
                    remaining -= count;
                }                
                ...
                while (remaining >= 4)
                {
                    ...
                    if (chunkLength  == 0)
                    {
                        throw new Exception($"The chunk must non zero size.");
                    }                    

                    if ((chunkLength & 3) != 0)
                    {
                        throw new Exception($"The chunk must be padded to 4 bytes: {chunkLength}");
                    }
                    ...
                    {
                        throw new Exception($"Duplicated chunk found {chunkId}");
                    }
                ...
                if (!chunks.ContainsKey(CHUNKJSON)) throw new Exception("JSON Chunk chunk not found");                
        ...
        private static long _ReadBinaryHeader(BinaryReader binaryReader)
        {
            ...
            if (magic != GLTFHEADER) throw new Exception($"Unexpected magic number: {magic}");

            uint version = binaryReader.ReadUInt32();
            if (version != GLTFVERSION2) throw new Exception($"Unknown version number: {version}");
            ...
            try // check stream is large enough
            {
                if (binaryReader.BaseStream.CanSeek)
                {
                    ...
                    {
                        throw new Exception($"The specified length of the file ({bodyLength}) is not equal to the actual length of the file ({fileLength}).");
                    }
                }
            }
